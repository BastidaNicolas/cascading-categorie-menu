...
const Categories = ({ categories, selectedCat, setSelectedCat }) => {
  ...
  const hasChildren = (category, parent) => {
    if (category.children) {
      return (
        <div className={`overflow-x-hidden min-w-[13.9375Rem] ${selected.some((item) => item.label === category.label) ? 'flex' : 'hidden'}`}>
          <ul
            className={
              `flex border-r-2 flex-col overflow-y-auto overflow-x-hidden min-w-[13.9375Rem]`
            }>
  ...
  return (
    <>
      <div className='border-2 rounded max-w-full w-full overflow-auto mb-5 '>
        <div className=' flex flex-col w-fit overflow-auto'>
          <div className='h-64 flex overflow-x-auto'>
            <ul className='flex flex-col border-r-2 overflow-y-auto overflow-x-hidden min-w-[13.9375Rem]'>
